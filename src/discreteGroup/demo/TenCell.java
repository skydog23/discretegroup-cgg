...
public class TenCell extends LoadableScene  {
	...
	transient double implodeTri =  .25,
			implodeHex = .25,
			scaleShape = .382,
			beamRadius = .05,
			tubeRadius = .015,
	...
	transient private Color[] 		colorList = new Color[] {
			new Color(0, 240, 250), //Color.blue,
			Color.green,
			Color.red,
	...
	transient int whichAnim = 0;		// 0 = none
	transient 	private TextSlider scaleSlider, tubeRadSlider, metricSlider;
	private WingedEdge fundamentalDomainWE = new WingedEdge();
...
//		scene.getAppearance().setAttribute(CommonAttributes.OFFSET, new double[]{0,0,0});
		...
		scene.getAppearance().setAttribute(CommonAttributes.AMBIENT_COEFFICIENT, .05);
		scene.getAppearance().setAttribute(CommonAttributes.DIFFUSE_COEFFICIENT, 1);
		scene.getAppearance().setAttribute(CommonAttributes.SPECULAR_COEFFICIENT, .1);
		...
		if (whichAnim == 1) scene.addChild(singleAnimatedDirdom);
//		allLines.getAppearance().setAttribute(CommonAttributes.LIGHTING_ENABLED, false);
//		allLines.getAppearance().setAttribute(CommonAttributes.SMOOTH_SHADING, false);
		...
		allLines.getAppearance().setAttribute(CommonAttributes.LINE_WIDTH, 6.0);
...
//		RopeTextureFactory stf = new RopeTextureFactory(new Appearance()); //.getAppearance());
//		stf.setN(15);
//		stf.setM(1);
...
//		moo.assignFrom(Rn.diagonalMatrix(null, new double[]{-1,-1,-1,-1}));
//		moo.assignTo(polar);
//		scene.addChildren(id, polar, lines);
//		for (int i = 0; i<2; ++i)	{
//			listOfTen[i].getAppearance().setAttribute("polygonShader.diffuseColor", colorList[0]);
//			listOfTen[i].getAppearance().setAttribute("lineShader.diffuseColor", colorList[0]);			
//		}
//		if (!showRingOfSix) {
...
//		}
		...
		SceneGraphComponent helixCurveSGC = SceneGraphUtility.createFullSceneGraphComponent("helix");
//		helixCurveSGC.getAppearance().setAttribute(CommonAttributes.TUBES_DRAW, true);
		helixCurveSGC.getAppearance().setAttribute("lineShader.diffuseColor", Color.yellow);
		helixCurveSGC.getAppearance().setAttribute(CommonAttributes.EDGE_DRAW, true);
		helixCurveSGC.getAppearance().setAttribute(CommonAttributes.LINE_WIDTH, 5.0);
		helixCurveSGC.getAppearance().setAttribute(CommonAttributes.TUBE_RADIUS, .05);
		helixCurveSGC.getAppearance().setAttribute(CommonAttributes.LIGHTING_ENABLED, false);
		stf = new SimpleTextureFactory();
		stf.setType(TextureType.STRIPES);
		stf.update();
		stf.setColor(0, Color.white);
		stf.setColor(1, new Color(0, 125, 255));
		stf.setParams(new double[]{.3, .5, 0.8, 1.0});
		stf.setIndices(new int[]{0,1,0,1});
		stf.update();
		tex2d = TextureUtility.createTexture(curveSGC.getAppearance(), "polygonShader", stf.getImageData());
		Matrix foo = new Matrix();
		MatrixBuilder.euclidean().rotateZ(Math.PI/2.0).assignTo(foo);
		tex2d.setTextureMatrix(foo);
		helixCurveSGC.getAppearance().setAttribute(CommonAttributes.AMBIENT_COEFFICIENT, .2);
//		helixCurve.addPoint(P3.originP3);
		helixCurveSGC.setGeometry(helixCurve.getCurve());
		scene.addChild(helixCurveSGC);
//		other5 = SceneGraphUtility.createFullSceneGraphComponent("other 5");
//		other5.addChild(scene);
//		double[] m = P3.makeScrewMotionMatrix(null, new double[]{0,0,0,2}, new double[]{1,1,1,1}, Math.PI/4.0, Pn.ELLIPTIC);
//		Rn.times(m,m,m);
...
//		SceneGraphComponent all = SceneGraphUtility.createFullSceneGraphComponent("all");
	...
	boolean doStripes = true;
	double[] params3 = {.5,.75, 1};
	int[] indices3 = {2,0,1};
	double[] params4 = {.5, .666, .8333, 1.0};
	int[] indices4 = {3,0,1,2};
...
//					   m.setEntry(3,1, -1);
	...
	double[][] centerPoints = new double[5][];
	PointRangeFactory[] axisFactories = new PointRangeFactory[10], polarFactories = new PointRangeFactory[10];
	{
	...
	protected void update() {
		...
		kiteSGC.setGeometry(kite);
		IndexedFaceSet tri = IndexedFaceSetUtility.implode(triFactory.getIndexedFaceSet(), implodeTri);
...
//			P3.orthonormalizeMatrix(ferd.getArray(), ferd.getArray(), 10E-6, Pn.ELLIPTIC);
		...
		for (int j = 0; j<5; ++j)	{
			for (int k = j+1; k<5; ++k)	{
				axisFactories[i].setElement0(centerPoints[j]);
				axisFactories[i].setElement1(centerPoints[k]);
				axisFactories[i].update();
	...
	protected void updateAxisPairs() {
		for (int i = 0; i<10; ++i)	{
			axisList[i].setGeometry(axisFactories[i].getTubedLine(null, tubeRadius, 24).getGeometry()); //getLine());//
			polarAxisList[i].setGeometry(polarFactories[i].getTubedLine(null, tubeRadius, 24).getGeometry()); //getLine());
		}
	}
...
//		ap.setAttribute(CommonAttributes.TUBES_DRAW, false);
		...
		ap.setAttribute(CommonAttributes.USE_GLSL, false);
		SimpleTextureFactory stf = new SimpleTextureFactory();
...
//		dgsgr.setAppList(aps);
	...
	SceneGraphComponent movingScrew = SceneGraphUtility.createFullSceneGraphComponent("moving screw");
...
//		viewer.getSceneRoot().getAppearance().setAttribute(CommonAttributes.RENDER_S3, true);
		...
		apl.setAnimateSceneGraph(true);
		switch (whichAnim)	{
		case 0:
			break;
		case 1:
			Animated an = new AnimatedAdaptor() {
//				@Override
...
//		apl.getAnimated().add(ab);					
	...
	public Component getInspector(Viewer viewer) {
	    ...
	    for (int i = 0; i<10; ++i)	{
	    	...
	    	vbox.add(hbox);
	    	for (int j = 0; j<4; ++j) {
	    		bloops[j][i] = new JCheckBox(labels[j]+(j<2 ? i : suffix[i]));
	    		bloops[j][i].setSelected(bigArray[j][i].isVisible());
				archive[j][i] = bigArray[j][i].isVisible();
	    		hbox.add(bloops[j][i]);
	    		final int row = i, column = j;
	    		bloops[j][i].addActionListener(new ActionListener() {
...
//		beamRadius = (originalMetricFactor/metricFactor) * t;
	...
	public SceneGraphComponent makeLights() {
		...
		double s = 1;
		double[][] positions = {{-1,-1,-1,s},{1,-1,1,s},{-1,1,1,s}, {1,1,-1,s}};
...
//	double[] mm = P3.makeScrewMotionMatrix(null, pt1, pt2, dangle * Math.PI, Pn.ELLIPTIC);
	...
	private TextSlider impHexSlider;
	private TextSlider impTriSlider;
	transient 	boolean animating = false;
	AnimatedAdaptor anim1 = new AnimatedAdaptor()	{
		transient 	double[] keytimes = {.5,1.5,2.5};
		@Override
