...
public class SchatzCube extends Assignment {
	...
	transient PointCollector oloidCenterCollector = new PointCollector(500, 4);
	transient PointRangeFactory axis1Factory;
	enum Geomtype {SCHATZ, TETRA, STICK, REGULUS, HEXAGON};
	transient SceneGraphPath w2FixedEdge, w2Diagonal, w2M1, w2M2;
...
//		polarize = false,
	...
	transient double[][] verts = {
			...
			{0,2,0,1},
			{0,0,0,1}};
	transient double schatzEdgeCenter[];
	...
	private double[] schatzMotion, moved2Still;
	...
	transient Matrix stillDiagonal = new Matrix();
	transient double[] zdir = {0,0,1,0}, origin = {0,0,0,1};
	@Override
	public SceneGraphComponent getContent() {
		...
		oloidCenterSGC = SceneGraphUtility.createFullSceneGraphComponent("oloid center");
		...
		// this is for the polarized surface: java code generated by mathematica, hence the name
...
//				System.err.println("t = "+angle);
		...
		w2Diagonal = new SceneGraphPath();
		w2Diagonal.push(world);
		w2Diagonal.push(otherStuffSGC);
		w2Diagonal.push(diagonalSGC);
...
//		otherStuffSGC.addChild(sgc);
		prf = new PointRangeFactory();
		prf.setElement0(origin);
		prf.setElement1(zdir);
		prf.setFiniteSphere(true);
		prf.setSphereRadius(5.0);
		prf.setNumberOfSamples(6);
		prf.update();
...
//		deltoidSGC.getAppearance().setAttribute(CommonAttributes.EDGE_DRAW, true)
		...
		diagonalFactory = 
				IndexedLineSetUtility.createCurveFactoryFromPoints(
						new double[][]{deltoidVerts[0], deltoidVerts[3]}, false);
		diagonalSGC.setGeometry(diagonalFactory.getGeometry());
		diagonalSGC.setVisible(showDiagonal || showOloid || showOloid2);

		stillDiagonal = generateBasisMatrix(deltoidVerts[0], deltoidVerts[3]);
...
//		setA(angle);
		...
		ap = oloidCenterSGC.getAppearance();
		ap.setAttribute("lineShader.diffuseColor", new Color(100,255,50));
		ap.setAttribute("lineShader.polygonShader.diffuseColor", new Color(100,255,50));
		otherStuffSGC.addChildren(zaxisSGC, deltoidGridSGC, inverseDeltoidSGC, diagonalSGC, axisHistorySGC, deltoidSGC, oloidCenterSGC);
		...
		oloidSGC.setTransformation(schatzLinkSGC.getChildComponent(1)
				.getTransformation());
//		oloidSGC.addChild(oloidCenterSGC);
		oloidCenterSGC.setGeometry(oloidCenterCollector.getCurve());
...
//		dgSGC.setTransformation(geomSGC.getTransformation());
	...
	private Matrix generateBasisMatrix( double[] p1, double[] p2) {
		Matrix m = new Matrix();
		double[] line = PlueckerLineGeometry.lineFromPoints(null,p1, p2);
		double[] polarline = PlueckerLineGeometry.polarize(null, line, Pn.EUCLIDEAN);
		double[] plane = P3.planeFromPoints(null, p1, p2, zdir),
				normal = PlueckerLineGeometry.lineIntersectPlane(null, polarline, plane);
		m.setColumn(0, Pn.normalizePlane(null, normal, Pn.EUCLIDEAN));
		m.setColumn(1, Pn.normalizePlane(null, Pn.polarize(null, plane, Pn.EUCLIDEAN), Pn.EUCLIDEAN));
		m.setColumn(2, p1);
		m.setColumn(3, p2);
		return m;
	}
	...
	private void setupGeometry() {
		...
		schatzEdgeCenter = Rn.times(null, .5, Rn.add(null, schatzverts[0], schatzverts[2]));
//		double[] tm = schatzLinkSGC.getChildComponent(4).getTransformation().getMatrix();
//		schatzEdgeCenter = Rn.matrixTimesVector(null, tm, schatzEdgeCenter);
	...
	private void setA(double unsymmetricA)	{
		...
		mm.assignTo(oloidProper);	// oloid is centered on one bar of the Schatz linkage, so moves with this bar
		updateDeltoid();
...
//				Rn.subtract(null,fiveDeltoidVerts, Pn.dehomogenize(tformedV, tformedV))));
		...
		// calculate matrix needed to hold the diagonal still
		Matrix movedDiagonal = generateBasisMatrix(diagverts[0], diagverts[1]);
		System.err.println("\nd = "+Pn.distanceBetween(diagverts[0], diagverts[1], Pn.EUCLIDEAN));
...
//		System.err.println("moved diagonal = \n"+Rn.matrixToString(movedDiagonal.getArray()));
		System.err.println("moved2still = \n"+Rn.matrixToString(moved2Still));
//		double[] am = Rn.times(null, moved2Still, movedDiagonal.getArray());
//		System.err.println("calculated still = \n"+Rn.matrixToString(am));
//		System.err.println("real still = \n"+Rn.matrixToString(stillDiagonal.getArray()));
...
//		System.err.println("center point = "+Rn.toString(center));
		...
		// collect the center of first bar in world coordinate system
//		double[] cumulM = Rn.times(null,moved2Still, schatzMotion);
		double[] cumulM =  moved2Still;
		schatzEdgeCenter = Rn.times(null, .5, Rn.add(null,deltoidVerts[1], deltoidVerts[2] ));
		double[] tmp = Rn.matrixTimesVector(null, 
				cumulM, schatzEdgeCenter);
		if (count % 3 == 0)  
			oloidCenterCollector.addPoint(tmp);
//	    oloidCenterSGC.getTransformation().setMatrix(Rn.inverse(null, moved2Still));
		count++;

      	w2FixedEdge.getMatrix(m1);
		double[] im1 = Rn.inverse(null, m1);
		if (fixBar) superworld.getTransformation().setMatrix(moved2Still); //im1);
...
//				updateOloidExtendedOloid();
		...
		fixCoordCB.setSelected(fixBar);
		fixCoordCB.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				fixBar = fixCoordCB.isSelected();
				if (fixBar) superworld.getTransformation().setMatrix(w2FixedEdge.getInverseMatrix(null));
				else superworld.getTransformation().setMatrix(Rn.identityMatrix(4));
			}	
		});
...
//		tex2d.setAnimated(true);
		...
		oloidTextureFactory.setColor(1, salmon);
